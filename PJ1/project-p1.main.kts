...
// represents a flash card with a question and an answer
data class FlashCard(val front: String, val back: String)

val card1 = FlashCard("What's Tyler's favorite color?", "Yellow!")
val card2 = FlashCard("What year did the revolutionary war start?", "1776")
val card3 = FlashCard("What time is noon?", "12:00pm")
...
// represents a deck of flash cards, with a name and list of flash cards
data class Deck(val name: String, val cards: List<FlashCard>)

val deck1 = Deck("first deck", listOf(card1, card2))
val deck2 = Deck("second deck", listOf(card2, card3))
...
// creates a list of flash cards for perfect squares from 1 to count
fun perfectSquares(count: Int): List<FlashCard> {
    // makes a flash card based off of the number supplied (normarlized for index vs real) and it's square
    fun kthPerfectSquare(x: Int): FlashCard {
        val y = x + 1
        return FlashCard(y.toString() + "^2 = ?", (y * y).toString())
    }
    return List<FlashCard>(count, ::kthPerfectSquare)
}

@EnabledTest
fun testPerfectSquares() {
    testSame(perfectSquares(0), emptyList<FlashCard>(), "empty")
    testSame(perfectSquares(3), listOf(FlashCard(square1Front, square1Back), FlashCard(square2Front, square2Back), FlashCard(square3Front, square3Back)))
}
...
// converts a flash card to a string with a charSep in between the question and answer
...
fun testCardToString() {
    testSame(cardToString(card1), "What's Tyler's favorite color?|Yellow!", "card1")
    testSame(cardToString(card2), "What year did the revolutionary war start?|1776", "card2")
    testSame(cardToString(card3), "What time is noon?|12:00pm", "card3")
}
...
// converts a properly formatted string into a flashcard
fun stringToCard(s: String): FlashCard {
    return FlashCard(s.substring(0, s.indexOf("|")), s.substring(s.indexOf("|") + 1))
}

@EnabledTest
fun testStringToCard() {
    testSame(stringToCard("What's Tyler's favorite color?|Yellow!"), card1, "card1")
    testSame(stringToCard("What year did the revolutionary war start?|1776"), card2, "card2")
    testSame(stringToCard("What time is noon?|12:00pm"), card3, "card3")
}
...
// reads txt file and uses list abstractions to create a flash card list
fun readCardsFile(path: String): List<FlashCard> {
    if (fileExists(path)) {
        return (fileReadAsList(path).map(::stringToCard))
    }
...
fun testReadCardsFile() {
    testSame(readCardsFile("hi!"), emptyList<FlashCard>(), "empty")
    testSame(readCardsFile("example.txt"), listOf(FlashCard("front 1", "back 1"), FlashCard("front 2", "back 2")), "exampletxt")
}
...
// checks if the inputted string starts with the letter Y/y
fun isPositive(s: String): Boolean {
    return s.uppercase().first().equals('Y')
}
...
// takes in a list of choices and returns a formatted numbered string of choices
fun choicesToText(list: List<String>): String {
    fun makeList(index: Int): String {
        return (index + 1).toString() + ". " + list[index]
    }
    val formattedList = List<String>(list.count(), ::makeList)
    val formattedString = linesToString(formattedList)
    return linesToString(formattedString, "", promptMenu)
}
...
// returns the deck name of the supplied deck
...
@EnabledTest
fun testGetDeckName() {
    testSame(getDeckName(deck1), "first deck", "first")
    testSame(getDeckName(deck2), "second deck", "second")
}

// returns the correct index for the user supplied number (userNum-1), if the initial input isn't an int or is outside of range returns -1
fun keepIfValid(input: String, range: IntRange): Int {
    when {
        !isAnInteger(input) -> return -1
        range.contains(input.toInt() - 1) -> return (input.toInt() - 1).toInt()
        else -> return -1
    }
}
...
@EnabledTest
fun testKeepIfValid() {
    testSame(keepIfValid("1", 0..2), 0, "1")
    testSame(keepIfValid("quit", 0..2), -1, "quit")
    testSame(keepIfValid("4", 0..2), -1, "4")
}

// returns a formatted string based on the inputted string
...
fun testChoiceAnnouncement() {
    testSame(choiceAnnouncement("name"), "you chose: name", "name")
    testSame(choiceAnnouncement("howdy"), "you chose: howdy", "howdy")
}
...
fun chooseOption(decks: List<Deck>): Deck {
    ...
    //  returns a string with all of the deck options
    ...
    // checks to see if the user made a valid deck choice, and changes it to that option if valid
    ...
    // converts the current choice to the name of the deck
...
fun testChooseOption() {
    ...
    testSame(
        ...
        CapturedResult(
            ...
            "",
            "Enter your choice",
            "you chose: first deck",
        ),
        "deck1 (1 input)",
    )
    testSame(
        captureResults(::testHelp, "hey", "2"),
        CapturedResult(
            deck2,
            "1. first deck", "2. second deck", "", "Enter your choice", "1. first deck", "2. second deck", "", "Enter your choice", "you chose: second deck",
        ),
        "deck2 (2 input)",
    )
}
...
// stores an index, card side, and correct answers in a StudyState
data class StudyState(val cardIndex: Int, val isBack: Boolean, val correctAns: Int)

val study1 = StudyState(0, true, 4)
val study2 = StudyState(1, false, 5)
...
// runs a react console that allows the user to study a deck, returns the number of self-reported correct answers
fun studyDeck(deck: Deck): Int {
    // converts the current study state into a string (essentially prints the flashcard text from the current side)
    fun studyToText(state: StudyState): String {
        when (state.isBack) {
            true -> return linesToString(deck.cards[state.cardIndex].back, studyCheck)
            false -> return linesToString(deck.cards[state.cardIndex].front, studyThink)
        }
    }

    // takes the current state and input and returns a new state
    fun nextStudy(state: StudyState, input: String): StudyState {
        when {
            state.isBack == true && input.equals("Y") -> return StudyState(state.cardIndex + 1, false, state.correctAns + 1)
            state.isBack == true && input.equals("N") -> return StudyState(state.cardIndex + 1, false, state.correctAns)
            state.isBack == false -> return StudyState(state.cardIndex, true, state.correctAns)
            else -> return nextStudy(state, input())
        }
    }

    // checks to see if the state has finished the deck
    fun isDone(state: StudyState): Boolean {
        if (state.cardIndex > (deck.cards.count() - 1) && state.isBack == false) {
            return true
        }
        return false
    }

    // produces an end string that says the correct amount of answers
    fun endStudyToText(state: StudyState): String {
        return "You got ${state.correctAns} right!"
    }

    return reactConsole(
        initialState = StudyState(0, false, 0),
        stateToText = ::studyToText,
        nextState = ::nextStudy,
        isTerminalState = ::isDone,
        terminalStateToText = ::endStudyToText,
    ).correctAns
}
...
fun testStudyDeck() {
    ...
    testSame(
        captureResults(::testDeckOne, "", "Y", "", "N"),
        CapturedResult(
            1,
            "What's Tyler's favorite color?",
            studyThink,
            "Yellow!",
            studyCheck,
        ...
        "test deck one with 1 correct",
    )

    testSame(
        captureResults(::testDeckTwo, "", "Y", "", "Y"),
        CapturedResult(
            2,
            "What year did the revolutionary war start?",
            studyThink,
...
fun chooseAndStudy(): Int {
    ...
    val deckOptions = listOf(
        ...
        Deck("fromFile", readCardsFile("example.txt")),
        // deck from code via perfectSquares
        Deck("fromFunction", perfectSquares(5)),
        // deck hand-coded
        Deck("handCoded", listOf(FlashCard("2+3", "5"), FlashCard("3+3", "6"), FlashCard("5+10", "15"))),
    )
...
fun testChooseAndStudy() {
    testSame(
        captureResults(::chooseAndStudy, "3", "", "Y", "", "Y", "", "Y"),
        CapturedResult(
            3,
            "1. fromFile",
            "2. fromFunction",
        ...
        "hand coded with all right",
    )
    testSame(
        captureResults(::chooseAndStudy, "2", "", "N", "", "Y", "", "N", "", "Y", "", "Y"),
        CapturedResult(
            3,
            "1. fromFile",
            "2. fromFunction",
