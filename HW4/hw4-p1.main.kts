...
// student which contains a first/last name and an id number
data class Student(val first: String, val last: String, val id: Int)

// passes each student in the given list into a supplied function
fun myForEachDo(list: List<Student>, function: (s: Student) -> Unit) {
...
// prints a triangle for the amount of lines given
fun printTriangle(maxLines: Int) {
    for (x in 1..maxLines) {
        for (y in 1..x) {
            print("*")
        }
